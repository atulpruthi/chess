...
class PuzzleService {
     ...
     */
    async getRandomPuzzle(userId, difficulty) {
        try {
            ...
            const result = await database_1.default.query(`SELECT p.* FROM puzzles p
         ...
         ORDER BY RANDOM()
         LIMIT 1`, [minRating, maxRating, userId]);
            if (result.rows.length === 0) {
                // If all puzzles solved, get any puzzle in range
                const anyResult = await database_1.default.query(`SELECT * FROM puzzles 
           WHERE rating BETWEEN $1 AND $2
           ORDER BY RANDOM()
           LIMIT 1`, [minRating, maxRating]);
    ...
    async getPuzzleById(puzzleId) {
        try {
            const result = await database_1.default.query('SELECT * FROM puzzles WHERE id = $1', [puzzleId]);
            if (result.rows.length === 0)
                return null;
            return this.mapPuzzle(result.rows[0]);
        }
     ...
     */
    async submitAttempt(userId, puzzleId, attemptData) {
        try {
            ...
            // Record attempt
            await database_1.default.query(`INSERT INTO puzzle_attempts 
         (user_id, puzzle_id, solved, attempts, time_spent, rating_before, rating_after, rating_change)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [userId, puzzleId, solved, attempts, timeSpent, oldRating, newRating, ratingChange]);
            // Update puzzle stats
            await database_1.default.query(`UPDATE puzzles 
         SET nb_plays = nb_plays + 1,
             nb_solved = nb_solved + $1,
             solution_rate = ROUND((nb_solved::numeric + $1) / (nb_plays::numeric + 1) * 100, 2)
         WHERE id = $2`, [solved ? 1 : 0, puzzleId]);
            ...
            await database_1.default.query(`INSERT INTO user_puzzle_stats (user_id, puzzle_rating, puzzles_attempted, puzzles_solved, current_streak, best_streak, total_time_spent, last_puzzle_at)
         ...
         ON CONFLICT (user_id) DO UPDATE SET
           ...
           last_puzzle_at = NOW(),
           updated_at = NOW()`, [userId, newRating, solved ? 1 : 0, newStreak, bestStreak, timeSpent]);
            return {
                success: true,
                ratingChange,
                newRating
    ...
    async getUserStats(userId) {
        try {
            const result = await database_1.default.query('SELECT * FROM user_puzzle_stats WHERE user_id = $1', [userId]);
            if (result.rows.length === 0) {
     ...
     */
    async getPuzzlesByTheme(theme, limit = 10, offset = 0) {
        try {
            const result = await database_1.default.query(`SELECT * FROM puzzles 
         WHERE $1 = ANY(themes)
         ORDER BY rating
         LIMIT $2 OFFSET $3`, [theme, limit, offset]);
    ...
    async getThemes() {
        try {
            const result = await database_1.default.query('SELECT * FROM puzzle_themes ORDER BY difficulty_min');
            return result.rows;
        }
        catch (error) {
     ...
     */
    verifySolution(fen, userMoves, correctMoves) {
        try {
    ...
    async generateDailyPuzzle() {
        try {
            ...
            const result = await database_1.default.query(`SELECT id FROM puzzles 
         WHERE rating BETWEEN 1400 AND 1800
         AND solution_rate >= 50
         ORDER BY RANDOM()
         LIMIT 1`);
            if (result.rows.length > 0) {
                await database_1.default.query('INSERT INTO daily_puzzles (puzzle_id, date) VALUES ($1, CURRENT_DATE) ON CONFLICT (date) DO NOTHING', [result.rows[0].id]);
            }
        }
        catch (error) {
...
exports.puzzleService = new PuzzleService();
exports.default = PuzzleService;
