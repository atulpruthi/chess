...
class AchievementService {
   ...
   */
  async syncUserAchievements(userId: number): Promise<string[]> {
   ...
   */
  async syncAllUserAchievements(): Promise<{ userId: number; achievements: string[] }[]> {
    try {
      // Get all users
      const usersResult = await pool.query('SELECT id FROM users');
      const results: { userId: number; achievements: string[] }[] = [];
