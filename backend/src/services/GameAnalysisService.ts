...
import { getStockfishInstance } from './StockfishService';
...
class GameAnalysisService {
  ...
  async analyzeGame(gameId: number, depth: number = 20): Promise<GameAnalysisResult> {
    ...
    try {
      ...
      const gameResult = await client.query(
        'SELECT pgn, white_player_id, black_player_id FROM games WHERE id = $1',
        [gameId]
      );
      ...
      await client.query(
        `UPDATE game_analysis SET
          ...
          updated_at = NOW()
        WHERE id = $5`,
        [opening.name, opening.eco, accuracyWhite, accuracyBlack, analysisId]
      );
   ...
   */
  async getGameAnalysis(gameId: number) {
  ...
  private calculateAccuracy(moves: MoveEvaluation[]): number {
    ...
    for (const move of moves) {
      if (move.isBookMove) continue; // Skip book moves
   ...
   */
  private async detectOpening(chess: Chess): Promise<{ name: string; eco: string }> {
    // Get first 10 moves
    ...
    for (let i = 0; i < Math.min(history.length, 20); i++) {
      ...
      const result = await pool.query(
        'SELECT opening_name, eco_code FROM opening_book WHERE fen LIKE $1 || \'%\' LIMIT 1',
        [fen]
      );
   ...
   */
  private uciToSan(fen: string, uci: string | null): string | null {
    if (!uci) return null;
...
export default new GameAnalysisService();
