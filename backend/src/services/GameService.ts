...
class GameService {
   ...
   */
  async saveCompletedGame(gameResult: GameResult): Promise<SavedGame> {
    const client = await query('BEGIN');

    try {
      // Get current player games count for K-factor calculation
      const whiteStats = await query(
        'SELECT games_played FROM user_statistics WHERE user_id = $1',
        [gameResult.whitePlayer.userId]
      );
      const blackStats = await query(
        'SELECT games_played FROM user_statistics WHERE user_id = $1',
        [gameResult.blackPlayer.userId]
      );
      ...
      if (gameResult.isRated) {
        const ratingResult = eloService.calculateNewRatings(
      ...
      if (gameResult.isRated) {
        await query(
          ...
          [whiteNewRating, parseInt(gameResult.whitePlayer.userId)]
        );
        await query(
          'UPDATE users SET rating = $1, updated_at = NOW() WHERE id = $2',
          [blackNewRating, parseInt(gameResult.blackPlayer.userId)]
        );
      }
      ...
      await this.updatePlayerStatistics(
        parseInt(gameResult.whitePlayer.userId),
        gameResult.winner === 'white' ? 'win' : gameResult.winner === 'draw' ? 'draw' : 'loss',
        gameResult.timeControl || 'blitz',
        whiteNewRating
      );
      ...
      await this.updatePlayerStatistics(
        parseInt(gameResult.blackPlayer.userId),
        gameResult.winner === 'black' ? 'win' : gameResult.winner === 'draw' ? 'draw' : 'loss',
        gameResult.timeControl || 'blitz',
        blackNewRating
      );
  ...
  private async updatePlayerStatistics(
    ...
    await query(updateQuery, [newRating, newWinStreak, newBestStreak, userId]);
  ...
  private async checkAchievements(userId: number): Promise<void> {
    ...
    const userRating = await query('SELECT rating FROM users WHERE id = $1', [userId]);
  ...
  async getMatchHistory(
    ...
    const [games, count] = await Promise.all([
      query(gamesQuery, params),
      query(countQuery, [userId, ...(timeControl ? [timeControl] : [])])
    ]);
