...
class UserStatsService {
   ...
   */
  async updateStatsAfterGame(gameResult: GameResult): Promise<void> {
    ...
    try {
      ...
      if (timeControl === 'bullet') bulletGames++;
      else if (timeControl === 'blitz') blitzGames++;
      else if (timeControl === 'rapid') rapidGames++;
      else if (timeControl === 'classical') classicalGames++;
   ...
   */
  async getGameHistory(userId: number, limit: number = 20, offset: number = 0) {
    const result = await pool.query(
      `SELECT 
        g.id,
        g.white_player_id,
   ...
   * Get leaderboard
   */
  async getLeaderboard(limit: number = 100, timeControl?: string) {
    let query = `
      SELECT 
        u.id,
        u.username,
  ...
  private async checkAchievements(
    ...
    // Check games played
    if (stats.gamesPlayed === 100) achievements.push('Century');
    
    // Check rating milestones
    if (stats.newRating >= 2000) achievements.push('Master');
    else if (stats.newRating >= 1600) achievements.push('Expert');
    else if (stats.newRating >= 1400) achievements.push('Rising Star');
    
    // Check win streak
    if (stats.currentWinStreak >= 5) achievements.push('Win Streak');
   ...
   * Bookmark a game
   */
  async bookmarkGame(userId: number, gameId: number, note?: string) {
    await pool.query(
      `INSERT INTO game_bookmarks (user_id, game_id, note)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, game_id) DO UPDATE SET note = $3`,
      [userId, gameId, note || null]
    );
  }
   ...
   */
  async getBookmarkedGames(userId: number) {
    const result = await pool.query(
      `SELECT 
        gb.*,
        g.white_player_id,
